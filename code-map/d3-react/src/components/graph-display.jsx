...
class Graph extends Component {
    static NODE_R = 4;
    static USERIP = process.env.REACT_APP_USER_IP || localhost;
    constructor(props) {
        super(props);
        this.state = {
            data: {
    ...
    _handleNodeClick = async node => {
        ...
        const { USERIP } = Graph;
        const { data: posts } = await axios.post(`http://${USERIP}:6996/api/add_info`, position);
        this.sleep(1000);
    ...
    _handleNodeRightClick = async node => {
        ...
        const { USERIP } = Graph;
        await axios.post(`http://${USERIP}:6996/api/jump_info`, position);
    };
    ...
    async componentDidMount() {
        ...
        const { USERIP } = Graph;
        const OK = await axios.get(`http://${USERIP}:6996/api/get_info`);
        console.log("Receive OK:", OK);
        this.sleep(1000);
        const { data: posts } = await axios.get(`http://${USERIP}:6996/api/get_info_next`);
