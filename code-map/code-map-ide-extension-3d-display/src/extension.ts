...
function getWebviewContent() {
	...
	const ip = process.env.USER_IP || "localhost";
	return `<!DOCTYPE html>
	<html lang="en">
	<head>
		<style type="text/css">
	...
	<body>
        ...
        <iframe src="http://${ip}:6886/" scrolling="no" width="100%" height="100%" frameBorder="0" ></iframe>
    </body>
    </html>`;
}
let NEXT_TERM_ID = 1;
function openCodeMapTerminal(answer: string) {
	if ("FOLDER" === answer) {
...
function openFile(filePath: string, openfile: boolean) {
	try {
		//打开文本
...
function SetVscodeFolder() {
	...
	if (vsrootPath) {
		...
		const resultFileInProject: string = path.join(vscodefile, "result");
		const contralFileInProject: string = path.join(vscodefile, "contralmarks.json");
		const jumpFileInProject = path.join(vscodefile, "jumpmarks.json");
		const contralShowInnerFlag: string = path.join(vscodefile, "contralShowInnerFlag.json");
...
async function exeCompileFileSelfDefine(terminal: vscode.Terminal) {
	let vsrootPath = vscode.workspace.rootPath;
	if (vsrootPath) {
		const slefCompileFile = path.join(vsrootPath, "codemap_compile.txt");
		const vscodefile: string = path.join(vsrootPath, ".vscode");
		if (fs.existsSync(slefCompileFile) && fs.statSync(slefCompileFile).isFile()) {
			...
			const resultInProject: string = path.join(vscodefile, "result");
			const resultFileInProject: string = path.join(resultInProject, "compile_result");
			const Dot2JsonFileInProject: string = path.join(resultInProject, "Dot2Json.py");
			const SetEdgeForceFileInProject: string = path.join(resultInProject, "SetEdgeForce.js");
			const contralShowInnerFlag: string = path.join(vscodefile, "contralShowInnerFlag.json");
...
async function exeCompileFile(terminal: vscode.Terminal) {
	console.log("exeCompileFile");
	let vsrootPath = vscode.workspace.rootPath;
	if (vsrootPath) {
		const vscodefile: string = path.join(vsrootPath, ".vscode");
		const configure_file: string = path.join(vscodefile, "coufigure.txt");
		const compileFile: string = path.join(vscodefile, "compile.json");
...
function readCompileFile() {
	...
	if (vsrootPath) {
		...
		const slefCompileFile = path.join(vsrootPath, "codemap_compile.txt");
		const compileFile: string = path.join(vscodefile, "compile.json");
		const configureFile: string = path.join(vsrootPath, "configure");
		const makefileAm: string = path.join(vsrootPath, "Makefile.am");
		const cMakeLists: string = path.join(vsrootPath, "CMakeLists.txt");
...
function buildCompileJsonFileBase(mode: string, target_path: string, compile_file_path: string, config_file: string) {
...
function buildCompileJsonFile(mode: string, target_path: string, compile_file_path: string) {
